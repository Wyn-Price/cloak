...
public class CloakRenderChunk extends RenderChunk
{
	private final RenderChunk oldRender;
	private CloakChunkCache worldView;
	
	public CloakRenderChunk(World worldIn, RenderGlobal renderGlobalIn, int indexIn, RenderChunk render) 
	{
	...
	protected ChunkCache createRegionRenderCache(World world, BlockPos from, BlockPos to, int subtract) 
	{
		try
		{
			...
			worldView = new CloakChunkCache(world, from, to, subtract, oldCache);
			return worldView;
		} catch (Throwable e) {
			e.printStackTrace();
		}
